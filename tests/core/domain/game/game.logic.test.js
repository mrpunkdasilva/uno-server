  ...
  describe('Score Calculation', () => {
    describe('calculateCardPoints', () => {
      it('should return face value for number cards', () => {
        expect(calculateCardPoints({ type: 'number', value: '0' })).toBe(0);
        expect(calculateCardPoints({ type: 'number', value: '5' })).toBe(5);
        expect(calculateCardPoints({ type: 'number', value: '9' })).toBe(9);
      });

      it('should return 20 points for action cards', () => {
        expect(calculateCardPoints({ type: 'action', value: 'Skip' })).toBe(20);
        expect(calculateCardPoints({ type: 'action', value: 'Reverse' })).toBe(
          20,
        );
        expect(calculateCardPoints({ type: 'action', value: 'Draw Two' })).toBe(
          20,
        );
      });

      it('should return 50 points for wild cards', () => {
        expect(calculateCardPoints({ type: 'wild', value: 'Wild' })).toBe(50);
        expect(
          calculateCardPoints({ type: 'wild', value: 'Wild Draw Four' }),
        ).toBe(50);
      });

      it('should return 0 for invalid card types', () => {
        expect(calculateCardPoints({ type: 'invalid', value: 'Unknown' })).toBe(
          0,
        );
      });
    });
    ...
    describe('calculateHandScore', () => {
      ...
      it('should return 0 for null or undefined hand', () => {
        expect(calculateHandScore(null)).toBe(0);
        expect(calculateHandScore(undefined)).toBe(0);
      });

      it('should calculate total points for hand with only number cards', () => {
        const hand = [
          { type: 'number', value: '3' },
          { type: 'number', value: '7' },
          { type: 'number', value: '9' },
        ];
        expect(calculateHandScore(hand)).toBe(19); // 3 + 7 + 9
      });

      it('should calculate total points for hand with mixed cards', () => {
        const hand = [
          { type: 'number', value: '5' },
          { type: 'action', value: 'Skip' },
          { type: 'wild', value: 'Wild' },
        ];
        expect(calculateHandScore(hand)).toBe(75); // 5 + 20 + 50
      });

      it('should calculate total points for hand with all action cards', () => {
        const hand = [
          { type: 'action', value: 'Skip' },
          { type: 'action', value: 'Reverse' },
          { type: 'action', value: 'Draw Two' },
        ];
        expect(calculateHandScore(hand)).toBe(60); // 20 + 20 + 20
      });
    });

    describe('calculateFinalScore', () => {
      it('should return 0 for invalid game object', () => {
        expect(calculateFinalScore(null, 'winner-1')).toBe(0);
        expect(calculateFinalScore({}, 'winner-1')).toBe(0);
        expect(calculateFinalScore({ players: null }, 'winner-1')).toBe(0);
      });

      it('should calculate score from all losing players hands', () => {
        const game = {
          players: [
            {
              _id: 'winner-1',
        ...
        // Score = (5 + 3) + (20 + 50) = 78
        expect(calculateFinalScore(game, 'winner-1')).toBe(78);
      });

      it('should skip winner when calculating score', () => {
        const game = {
          players: [
            {
              _id: 'winner-1',
              hand: [
                { type: 'wild', value: 'Wild Draw Four' }, // Should not count
              ],
            },
            {
              _id: 'player-2',
        ...
        expect(calculateFinalScore(game, 'winner-1')).toBe(7);
      });

      it('should handle players with no hand', () => {
        const game = {
          players: [
            {
              _id: 'winner-1',
            ...
            {
              _id: 'player-3',
              hand: [{ type: 'number', value: '4' }],
            },
          ],
        };
        expect(calculateFinalScore(game, 'winner-1')).toBe(4);
      });

      it('should return 0 if only winner remains', () => {
        const game = {
          players: [
            {
              _id: 'winner-1',
        ...
        expect(calculateFinalScore(game, 'winner-1')).toBe(0);
      });
    });

    describe('buildScoreData', () => {
      it('should build score data object with correct structure', () => {
        const scoreData = buildScoreData('player-123', 'game-456', 100);
      ...
      it('should handle zero score', () => {
        const scoreData = buildScoreData('player-1', 'game-1', 0);
        expect(scoreData.score).toBe(0);
      });

      it('should handle large scores', () => {
        const scoreData = buildScoreData('player-1', 'game-1', 999);
        expect(scoreData.score).toBe(999);
      });
    });
  });
});
