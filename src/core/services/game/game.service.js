...
class GameService {
  ...
  async declareUno(gameId, userId) {
    return new CommonUtils.ResultAsync(GameDomain.validateGameId(gameId))
      .tap((trimmedId) => logger.info(`User ${userId} attempting to declare UNO in game ${trimmedId}`))
      .chain((trimmedId) => CommonUtils.fetchById(this.gameRepository, trimmedId, logger, 'game', new GameErrors.GameNotFoundError()))
      .chain(GameDomain.validateGameIsActive)
      .chain(GameDomain.validateUserInGame(userId))
      .chain(async (game) => {
        const player = game.players.find(p => p._id.toString() === userId.toString());
        
        // A player can only declare UNO if they have 1 or 2 cards left.
        if (player.hand.length > 2) {
          return CommonUtils.Result.failure(
            new GameErrors.CannotPerformActionError('You can only declare UNO with 1 or 2 cards remaining.')
          );
        }
        ...
        return CommonUtils.Result.success({ message: 'UNO declared successfully!' });
      })
      .tap(() => logger.info(`User ${userId} successfully declared UNO in game ${gameId}.`))
      .tapError((error) => logger.error(`Failed to declare UNO for user ${userId}: ${error.message}`))
      .getOrThrow();
  }
  ...
  async challengeUno(gameId, challengerId, targetPlayerId) {
    return new CommonUtils.ResultAsync(GameDomain.validateGameId(gameId))
      .tap((trimmedId) => logger.info(`User ${challengerId} challenging ${targetPlayerId} in game ${trimmedId}`))
      .chain((trimmedId) => CommonUtils.fetchById(this.gameRepository, trimmedId, logger, 'game', new GameErrors.GameNotFoundError()))
      .chain(GameDomain.validateGameIsActive)
      .chain(GameDomain.validateUserInGame(challengerId))
      .chain(async (game) => {
        const targetPlayer = game.players.find(p => p._id.toString() === targetPlayerId.toString());
        if (!targetPlayer) {
          return CommonUtils.Result.failure(new GameErrors.CannotPerformActionError('Target player not found in game.'));
        }

        // Challenge condition: Target has exactly 1 card and forgot to declare UNO
        if (targetPlayer.hand.length === 1 && targetPlayer.hasDeclaredUno === false) {
          const penaltyCards = [];
          for (let i = 0; i < 2; i++) {
        ...
        return CommonUtils.Result.failure(
          new GameErrors.CannotPerformActionError('Invalid challenge. Player is safe or has more than 1 card.')
        );
      })
      .tapError((error) => logger.error(`Challenge failed in game ${gameId}: ${error.message}`))
      .getOrThrow();
  }
    ...
    return new CommonUtils.ResultAsync(GameDomain.validateGameId(gameId))
      ...
      .chain(async (game) => {
        ...
        return CommonUtils.Result.success({
          gameId: game._id,
          status: game.status,
          turnDirection: game.turnDirection === 1 ? 'Clockwise' : 'Counter-clockwise',
          currentColor: game.currentColor,
          topCard: topCard,
          players: playersSnapshot
        });
      })
      .tapError((error) => logger.error(`Failed to get full game state for ${gameId}: ${error.message}`))
      .getOrThrow();
  }
}
